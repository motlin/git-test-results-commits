        ...
        Ok(output) => {
            if output.status.success() {
                let message = String::from_utf8_lossy(&output.stdout)
                    ...
                    .take(120)
                    .collect::<String>();
                debug!("Commit message retrieved: {}...", &message.chars().take(120).collect::<String>());
                message
            } else {
                error!("Error fetching commit message for {}: {}", treeish, String::from_utf8_lossy(&output.stderr));
